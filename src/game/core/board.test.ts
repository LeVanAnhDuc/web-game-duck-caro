...
describe('buildBoard', () => {
  it('ô chưa ai đánh là TRỐNG, không phải không hợp lệ', () => {
    const board = buildBoard([m(0, 0, 'one')]);
  ...
  it('giữ đúng bên cho từng ô, kể cả toạ độ âm', () => {
    const board = buildBoard([m(0, 0, 'one'), m(-1, -1, 'two'), m(2, -3, 'one')]);
    expect(markAt(board, { x: 0, y: 0 })).toBe('one');
    expect(markAt(board, { x: -1, y: -1 })).toBe('two');
    expect(markAt(board, { x: 2, y: -3 })).toBe('one');
    expect(isEmpty(board, { x: 0, y: 0 })).toBe(false);
  });

  it('nước sau ghi đè nước trước trên cùng một ô — chống lệch khi dựng lại', () => {
    const board = buildBoard([m(0, 0, 'one'), m(0, 0, 'two')]);
    expect(markAt(board, { x: 0, y: 0 })).toBe('two');
  });
});
...
describe('boundsOf', () => {
  ...
  it('bao được cả toạ độ âm', () => {
    expect(boundsOf([m(-2, 5, 'one'), m(3, -4, 'two')])).toEqual({
      minX: -2,
      minY: -4,
      maxX: 3,
      maxY: 5,
  ...
  it('một quân thì hộp bao là chính ô đó', () => {
    expect(boundsOf([m(7, -7, 'one')])).toEqual({
      minX: 7,
      minY: -7,
      maxX: 7,
      maxY: -7,
