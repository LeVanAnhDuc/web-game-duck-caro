...
import type { Level, Move, Point, Rule } from '@/game/core/types';
...
function movesFrom(rows: readonly string[]): Move[] {
  ...
  rows.forEach((row, y) => {
    [...row].forEach((ch, x) => {
      if (ch === 'x') out.push({ at: { x, y }, side: 'one' });
      if (ch === 'o') out.push({ at: { x, y }, side: 'two' });
    });
  });
  return out;
}
...
const paramsFor = (
  level: Level,
  seed: number,
  overrides: Partial<{ blindRate: number; depth: number; rule: Rule }> = {},
) => ({
  ...LEVELS[level],
  /* Luat mac dinh o day la caro Viet — bo test nay do MUC KHO, khong do luat. */
  rule: 'blocked' as Rule,
  ...overrides,
...
describe('mù có chủ đích — ADR-0005', () => {
  it('KHÔNG mù thì mọi mức đều chặn đúng ô duy nhất', () => {
    for (const level of ['easy', 'normal', 'hard'] as const) {
      const result = search(movesFrom(MUST_BLOCK), 'two', paramsFor(level, 7, { blindRate: 0, depth: 2 }));
  ...
  it('MÙ hoàn toàn thì bỏ qua nước chặn — đây là hành vi CỐ Ý, không phải bug', () => {
    ...
    const result = search(movesFrom(MUST_BLOCK), 'two', paramsFor('easy', 3, { blindRate: 1 }));
  ...
  it('mù rồi thì vẫn ăn được nước thắng của CHÍNH MÌNH — mù phòng thủ, không mù hẳn', () => {
    ...
    const board = ['.......', '.......', 'xoooo..', '.......'];
    const result = search(movesFrom(board), 'two', paramsFor('easy', 5, { blindRate: 1 }));
...
describe('tính tất định', () => {
  it('cùng seed, cùng thế bàn thì cùng một nước', () => {
    ...
    const a = search(board, 'two', paramsFor('hard', 11, { depth: 3 }));
    const b = search(board, 'two', paramsFor('hard', 11, { depth: 3 }));
    expect(a.at).toEqual(b.at);
  });

  it('mức Khó không phụ thuộc seed vì nó không có nhiễu', () => {
    const board = movesFrom(['..x....', '...o...', '..xo...', '.......']);
    const a = search(board, 'two', paramsFor('hard', 1, { depth: 3 }));
    const b = search(board, 'two', paramsFor('hard', 999, { depth: 3 }));
    expect(a.at).toEqual(b.at);
  });
});
